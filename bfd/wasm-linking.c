...
wasm_compute_symtab (bfd *abfd, bfd_size_type *size)
{
  ...
  for (i = 0; i < count; i++)
	  ...
	  && s->section->output_section != NULL)
	{
	  ...
	  unsigned int kind = wasm_symbol_type (s);
...
wasm_read_symtab (bfd *abfd, bfd_byte *start, bfd_byte *end)
{
  ...
  for (i = 0; i < (int) total; i++)
      ...
      else if (kind <= WASM_SYMTAB_TABLE)
	{
	  ...
	  sym->value = 0;
	  sym->flags = BSF_GLOBAL;
	  sym->section = bfd_wasm_get_section_by_number (abfd, objtype);
	  wsym->index = idx;
	  sym->udata.p = NULL;
	}
      else
	...
	continue;
      ...
      objsec = bfd_wasm_get_segment_by_index (abfd, objtype, wsym->index);
      if (! objsec)
	{
      ...
      sym->value += wasm_section_data (objsec)->offset;
    }
