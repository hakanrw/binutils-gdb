...
#define DECLARE_WASM_SECTION_OPS_SIZE_META_CONTENTS(NAME) \
  [WASM_SEC_##NAME] = { \
    .flatten = wasm_nsec_section_generic_flatten_size_meta_and_contents, \
    .reconstruct = wasm_nsec_section_generic_reconstruct_size_meta_and_contents, \
...
static void
wasm_nsec_flatten_compute_offsets (asection *asect, bool size_meta_contents)
{
  ...
  for (; curs; curs = curs->sibling_next)
    {
      if (size_meta_contents)
        pos += wasm_sizeof_uleb128 (wasm_nsec_subsec_len (curs->section)); /* Entry uleb size */
...
static void
wasm_nsec_flatten_merge_contents (asection *asect, bool size_meta_contents)
{
  wasm_section_tdata *sdata = wasm_section_data (asect);
  ...
  for (; curs; curs = curs->sibling_next)
    {
      ...
      bfd_vma curr_len = wasm_nsec_subsec_len (curs->section);

      if (size_meta_contents)
        wasm_write_uleb128_buf (curr_data - wasm_sizeof_uleb128 (curr_len), curr_len);
...
static int
wasm_nsec_section_generic_flatten_size_meta_and_contents (asection *sec)
{
  printf ("wasm_nsec_section_generic_flatten_size_meta_and_contents\n");
...
static void
wasm_nsec_reconstruct_section (asection *asect, bool size_meta_contents ATTRIBUTE_UNUSED)
{
  ...
  for (unsigned int i = 0; i < segcount; i++)
    {
      ...
      if (size_meta_contents)
        {
          ulen = wasm_read_uleb128_buf (cursor, end, &segsize);
          if (! ulen)
            {
              fprintf (stderr,
                       "wasm_nsec_reconstruct_section: failed to read ULEB128 for segment size "
                       "in section '%s' (offset 0x%zx)\n",
                       asect->name ? asect->name : "<unknown>",
                       (size_t)(cursor - asect->contents));
              return;
          ...
          cursor += ulen;
          if (cursor + segsize > end)
            {
              fprintf (stderr,
                       "wasm_nsec_reconstruct_section: segment size exceeds section "
                       "in section '%s' (offset 0x%zx)\n",
                       asect->name ? asect->name : "<unknown>",
                       (size_t)(cursor - asect->contents));
              return;
      ...
      int metalen = wasm_nsec_subsec_parse_meta (segsec, cursor, size_meta_contents ? cursor + segsize : end);
      if (metalen < 0)
        {
            fprintf (stderr,
...
static int
wasm_nsec_section_generic_reconstruct_size_meta_and_contents (asection *sec)
{
  printf ("wasm_nsec_section_generic_reconstruct_size_meta_and_contents\n");
  assert (sec->contents);
